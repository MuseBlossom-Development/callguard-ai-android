...
class CallGuardUseCase
    ...
    constructor(
         ...
         * 3. CDN URL 요청
         */
        suspend fun prepareCallAnalysis(onSilentSignIn: suspend () -> Result<String>): Result<CDNUrlData> =
         ...
         */
        suspend fun uploadAudioForAnalysis(audioFile: File): Result<String> =
            withContext(dispatcher) {
                try {
                    // 1. CDN URL 요청
                    val cdnResult = repository.getCDNUrl()
         ...
         * 보이스피싱 텍스트 전송
         */
        suspend fun sendVoicePhishingText(
            uuid: String,
            callText: String,
        ): Result<Unit> =
            withContext(dispatcher) {
